...
/** Exactly the columns toDoc() reads, per kind - so list reads project their
 *  columns instead of select('*'). Derived from KIND (not a hardcoded list) so it
 *  can't drift from toDoc; receipts carry class_snapshot, pay slips don't. */
function docColumns(k: (typeof KIND)[FinanceKind]): string {
  return [
    'id',
    'number',
    k.partyCol,
...
export async function selectDocPage(
  ...
  return { rows: ((data ?? []) as unknown as Record<string, unknown>[]).map((r) => toDoc(kind, r)), total: count ?? 0 }
}
