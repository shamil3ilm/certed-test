...
export function ArchivedList({
  ...
  return (
    <details className="text-sm">
      <summary className="cursor-pointer text-xs font-medium text-slate-600 transition hover:text-primary">
      ...
      <ul className="mt-2 space-y-2">
        {items.map((item) => (
          <li key={item.key} className={ARCHIVED_ROW}>
            ...
            {item.meta && <span className="shrink-0 text-xs text-slate-600">{item.meta}</span>}
            {item.action}
          </li>
        ))}
      </ul>
