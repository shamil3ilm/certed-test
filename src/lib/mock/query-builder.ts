...
export class MockQueryBuilder implements PromiseLike<Result> {
  ...
  private async exec(): Promise<Result> {
    ...
    if (this.op === 'delete') {
      ...
      const keep = this.rows.filter((r) => !removed.has(r))
      this.rows.length = 0
      this.rows.push(...keep)
      persist()
      // Honour `.select()` after `.delete()`, like insert/update/upsert already
      // do. PostgREST returns the deleted rows there, and callers use that to
      // tell a real delete from a no-op - returning null made every mock-mode
      // delete report "0 rows affected" while actually removing them.
      return this.returning ? this.shapeReturn(matched) : { data: null, error: null }
    }
