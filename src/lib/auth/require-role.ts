...
/**
 * API/Route-Handler guard for any signed-in active user. Use for self-service
 * endpoints whose actual boundary is ownership or a downstream record check,
 * rather than a fixed persona/capability gate at transport level.
 */
export async function requireActiveProfileApi(): Promise<Profile> {
  const actor = await getActorContext()
