 ...
 */
async function canViewReportCard(actor: ActorContext, studentId: string): Promise<boolean> {
...
export async function getReportCardData(actor: ActorContext, studentId: string): Promise<ReportCardData | null> {
  ...
  // Report cards are a student-only document. Reject any self-target or crafted
  // id that resolves to another active persona instead of emitting a blank or
  // misleading PDF for a non-student account.
