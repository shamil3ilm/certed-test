...
function authClientMessage(message: string): string {
  if (message === 'forbidden') return FORBIDDEN_MESSAGE
  if (message === 'revoked') return ACCESS_REVOKED_MESSAGE
  if (message === 'no-access') return NO_ACCESS_MESSAGE
...
export function authFail(error: unknown) {
  ...
  return fail(authClientMessage(msg), status, codeForAuthMessage(msg))
}

/** Plain-text counterpart to authFail for download and document endpoints. */
export function authTextFail(error: unknown) {
  const msg = error instanceof Error ? error.message : 'error'
  const status = msg === 'forbidden' || msg === 'revoked' ? 403 : 401
  return textFail(authClientMessage(msg), status)
}
