...
import { getCapabilityOverrides, GLOBALLY_NON_OVERRIDEABLE_CAPABILITIES } from '@/lib/services/capability-overrides'
...
export async function loadUserPermissionsView(actor: Profile, targetId: string): Promise<UserPermissionsView> {
  ...
  const globalPersonas = personas
    .filter((p) => p.scope_type === 'global' && p.status === 'active')
    .map((p) => ({ persona_name: p.persona_name }))
  const scopedMentorCount = personas.filter(
    (p) => p.persona_name === 'mentor' && p.scope_type !== 'global' && p.status === 'active',
  ).length

  const personaBaseline = globalPersonas.length > 0 ? globalPersonas : [{ persona_name: target.role }]
  const baseline = getBaseCapabilities(personaBaseline)
  const overrides = await getCapabilityOverrides(targetId)
  const resolved = resolveCapabilities({ personas: personaBaseline, overrides })
  ...
  const rows: PermissionRow[] = ALL_CAPABILITIES.map((capability) => {
    ...
    const isOverrideBlocked = GLOBALLY_NON_OVERRIDEABLE_CAPABILITIES.has(capability)
