...
import { classBanner } from '@/lib/ui'
...
export default async function ClassLayout({ params, children }: { params: { id: string }; children: ReactNode }) {
  ...
  return (
    <main className="mx-auto max-w-5xl p-4 sm:p-6 lg:p-8">
      ...
      <div
        className={`overflow-hidden rounded-2xl bg-gradient-to-br ${classBanner(course.id)} px-5 py-6 sm:px-7 sm:py-8`}
      >
        <p className="text-xs font-semibold uppercase tracking-wide text-white/70">
          {course.status === 'archived' ? 'Archived class' : 'Class'}
        </p>
        <h1 className="mt-1 text-xl font-bold tracking-tight text-white sm:text-2xl lg:text-3xl">{course.name}</h1>
      </div>
