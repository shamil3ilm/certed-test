...
export function MarkAttendanceForm({ classId, date, students }: { classId: string; date: string; students: Row[] }) {
  const serverSignature = students.map((row) => `${row.id}:${row.status ?? ''}`).join('|')

  return <MarkAttendanceFormBody key={serverSignature} classId={classId} date={date} students={students} />
}

function MarkAttendanceFormBody({ classId, date, students }: { classId: string; date: string; students: Row[] }) {
