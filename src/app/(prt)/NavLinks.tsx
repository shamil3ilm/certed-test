...
export function NavLinks({ links }: { links: NavItem[] }) {
  ...
  return (
    <nav className="hidden flex-wrap items-center justify-center gap-1 border-t border-slate-100 py-2 md:flex">
      {links.map((l, i) => {
        ...
        return (
          <div key={l.href} className="flex items-center">
            {startsGroup && <span className="mx-1.5 h-5 w-px bg-slate-200" aria-hidden="true" />}
            <Link
              href={l.href}
