...
import { issuePayslipSchema } from '@/lib/validation/finance'
import { issuePayslip } from '@/lib/finance/issue'
...
export async function POST(req: Request) {
  ...
  const parsed = issuePayslipSchema.safeParse(await req.json().catch(() => null))
  if (!parsed.success) return fail('invalid input', 422)
  try {
    const result = await issuePayslip(parsed.data, me.id)
    return ok(result)
  } catch (e) {
