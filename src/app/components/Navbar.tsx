                ...
                <div className="flex justify-between h-20 items-center">
                    <div className="flex-shrink-0 flex items-center">
                        <Link href="/" className="flex items-center">
                            <Image src="/cert-ed-academia-online-tuition-logo.webp" alt="Cert-Ed Academia logo for online tuition classes" width={320} height={80} className="h-14 w-auto object-contain" priority />
                        </Link>
                    </div>
                    ...
                    <div className="hidden md:flex space-x-4">
                        {navLinks.map((link) => (
                            <Link
                                key={link.name}
                                href={link.href}
                                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 border ${isActive(link.href)
                ...
                <div className="px-4 pt-2 pb-4 space-y-2">
                    {navLinks.map((link) => (
                        <Link
                            ...
                            className={`block px-4 py-3 rounded-lg text-base font-medium border transition-all duration-200 ${isActive(link.href)
