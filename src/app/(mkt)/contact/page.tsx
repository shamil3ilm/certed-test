...
export default function Contact() {
  ...
  useEffect(() => {
    return () => {
      if (successResetTimeoutRef.current) clearTimeout(successResetTimeoutRef.current)
    }
  }, [])
            ...
            {status === 'success' ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <button
                  type="submit"
                  disabled={status === 'loading'}
                  className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-4 px-6 rounded-lg transition-colors flex items-center justify-center gap-2 disabled:bg-primary/40 disabled:cursor-not-allowed"
                >
                  {status === 'loading' ? (
                    <>
