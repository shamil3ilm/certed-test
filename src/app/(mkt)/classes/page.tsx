  ...
  return (
    <div className="flex min-h-screen flex-col">
      <section className="bg-white px-4 py-10 text-center text-slate-900">
        <h1 className="mb-6 text-4xl font-bold text-slate-900 md:text-5xl">Online Classes & Curriculum</h1>
      ...
      <SectionWrapper id="class-overview" className="bg-slate-50">
        <div className="mb-8 text-center">
          <h2 className="mb-4 text-3xl font-bold text-slate-900">Class-wise Overview</h2>
        </div>
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
          {MARKETING_CLASS_LEVELS.map((level) => (
            <div
              key={level.title}
              className="group rounded-2xl border border-slate-100 bg-white p-8 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
            >
      ...
      <SectionWrapper id="how-it-works" className="bg-white">
        <div className="mb-8 text-center">
          <h2 className="text-3xl font-bold text-slate-900">How It Works - Our Process</h2>
          <p className="mt-2 text-lg text-slate-600">Simple, Effective, and Result-Oriented</p>
        </div>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {MARKETING_CLASS_STEPS.map((step, index) => (
            <div
              key={step.title}
              className="group relative flex h-full flex-col rounded-xl border border-slate-100 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
            >
              ...
              <h3 className="mb-3 text-lg font-bold text-slate-900">{step.title.replace(/^Step \d+ - /, '')}</h3>
              <p className="flex-grow text-sm leading-relaxed text-slate-600">{step.description}</p>
            </div>
          ))}
        </div>
      </SectionWrapper>
