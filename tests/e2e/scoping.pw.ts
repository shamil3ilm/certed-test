...
// role gates, and API class-scope gates (admin-any / teacher-own / global=admin).
...
const SEED = {
  math: 'c0000000-0000-4000-8000-000000000001',    // teacher teaches this
  science: 'c0000000-0000-4000-8000-000000000002',  // teacher teaches this too
}

async function loginAs(page: Page, email: string) {
  await page.context().clearCookies() // some tests re-login as another role within one test
...
test('page — teacher is blocked from admin areas', async ({ page }) => {
  await assertAdminBlocked(page, 'teacher@mock.test')
})
...
test('api — a teacher may create an event for a class they teach, but not a global one', async ({ page }) => {
...
test('api — a global event is editable by admin only (teacher/student get 403)', async ({ page }) => {
  ...
  await loginAs(page, 'teacher@mock.test')
  expect((await apiCall(page, 'PATCH', `/api/events/${id}`, { title: 'hijack' })).status, 'teacher PATCH global').toBe(403)
