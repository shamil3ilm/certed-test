...
import { apiError, authFail, authTextFail } from '@/lib/api/response'
...
describe('authFail', () => {
  it('maps coded auth errors to shared user-facing messages and stable codes', async () => {
    const unauthorized = await authFail(new Error('unauthorized')).json()
    ...
    const noAccess = await authFail(new Error('no-access')).json()
    expect(noAccess).toMatchObject({
      success: false,
      error: 'Your account does not currently have access to this resource.',
      code: ERROR_CODES.noAccess,
    })

    const revoked = await authFail(new Error('revoked')).json()
  ...
  it('provides the same user-facing auth message contract for plain-text endpoints', async () => {
    const res = authTextFail(new Error('forbidden'))
    expect(res.status).toBe(403)
    await expect(res.text()).resolves.toBe('You do not have access to this resource.')
  })
})
