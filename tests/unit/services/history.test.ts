...
vi.mock('@/lib/repos/audit', () => ({ listAuditPage: vi.fn() }))
...
describe('historyUrl', () => {
  it('builds the history URL while omitting the default page', () => {
    ...
    expect(historyUrl({ page: 2, action: 'grade', actor: 'maya' })).toBe('/admin/history?page=2&action=grade&actor=maya')
  })
})
...
describe('loadHistoryPageData', () => {
  ...
  it('forces a zero-row actor filter when the actor search matches nobody', async () => {
    vi.mocked(listProfiles).mockResolvedValueOnce([{ id: 'p1', full_name: 'Maya Mentor', email: 'maya@test.com' }] as any)
