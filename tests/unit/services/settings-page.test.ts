...
const tutor = { id: 'teach-1', email: 't@x.c', full_name: null, role: 'tutor', status: 'active', class_level: null, auth_user_id: 'auth-1' } as any
const student = { id: 'stud-1', email: 's@x.c', full_name: null, role: 'student', status: 'active', class_level: 'Grade 8', auth_user_id: 'auth-2' } as any
const tutorPersona = { persona_name: 'tutor', status: 'active' }
const studentPersona = { persona_name: 'student', status: 'active' }

beforeEach(() => vi.clearAllMocks())

describe('loadSettingsPageData', () => {
  it('maps saved and error query state into stable alert objects for non-student', async () => {
    vi.mocked(loadPersonaFlags).mockResolvedValueOnce({ personas: [], isAdmin: false, isSubAdmin: false, isTutor: true, isManager: true, isStudent: false, isMentor: false } as any)
    expect(
      await loadSettingsPageData(
        tutor,
        { saved: 'profile', error: 'password' },
        false,
      ),
    ).toEqual({
      alerts: [
  ...
  it('exposes student-specific class display and mock password help text', async () => {
    vi.mocked(loadPersonaFlags).mockResolvedValueOnce({ personas: [], isAdmin: false, isSubAdmin: false, isTutor: false, isManager: false, isStudent: true, isMentor: false } as any)
    expect(
      await loadSettingsPageData(
        student,
        { saved: 'password' },
