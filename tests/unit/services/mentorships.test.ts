...
describe('assignMentor / removeMentor are admin/sub_admin-only', () => {
  it('reject a tutor or student actor, without touching the DB', async () => {
    ...
    await expect(assignMentor(student, { tutorId: 'teach-1', studentId: 'stud-1' })).rejects.toBeInstanceOf(PermissionError)
  ...
  it('assignMentor rejects when the mentor id is not actually a tutor', async () => {
    ...
    await expect(assignMentor(admin, { tutorId: 'stud-2', studentId: 'stud-1' })).rejects.toBeInstanceOf(ValidationError)
    expect(createAdminClient).not.toHaveBeenCalled()
  })
  ...
  it('assignMentor rejects when the mentee id is not actually a student', async () => {
    ...
    await expect(assignMentor(admin, { tutorId: 'teach-1', studentId: 'teach-2' })).rejects.toBeInstanceOf(ValidationError)
    expect(createAdminClient).not.toHaveBeenCalled()
  })
  ...
  it('assigns and audits mentorship.assign for a valid tutor + student pair', async () => {
    ...
    await assignMentor(admin, { tutorId: 'teach-1', studentId: 'stud-1' })
  ...
  it('removes and audits mentorship.remove for an admin', async () => {
    vi.mocked(requireAdminOrSubAdminPersona).mockResolvedValueOnce(undefined)
    vi.mocked(createAdminClient)
      .mockReturnValueOnce(makeClient({ data: { tutor_id: 'teach-1', student_id: 'stud-1' }, error: null }) as any) // select + update
...
describe('mentorship action-input helpers', () => {
  ...
  it('rejects invalid mentorship action payloads with a typed validation error', () => {
    expect(() => validateAssignMentorInput({ tutor_id: 'bad', student_id: 'bad' })).toThrow(ValidationError)
