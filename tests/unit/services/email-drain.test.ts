...
vi.mock('@/lib/data/pending-emails', () => ({
  selectPendingEmails: vi.fn(),
  markEmailSent: vi.fn(),
  markEmailFailed: vi.fn(),
}))

import { emailEnabled, sendEmail } from '@/lib/email/resend'
import { selectPendingEmails, markEmailSent, markEmailFailed } from '@/lib/data/pending-emails'
import { drainPendingEmails } from '@/lib/services/email-drain'

beforeEach(() => vi.resetAllMocks())

describe('drainPendingEmails', () => {
  it('does nothing when email is disabled - the queue is left intact', async () => {
    ...
    expect(selectPendingEmails).not.toHaveBeenCalled()
  })

  it('sends each pending email and marks it sent', async () => {
    vi.mocked(emailEnabled).mockReturnValue(true)
    vi.mocked(selectPendingEmails).mockResolvedValue([
  ...
  it('retries a failed send, then parks it as failed once attempts are exhausted', async () => {
    ...
    expect(markEmailFailed).toHaveBeenCalledWith('fresh', 1, false, expect.any(String)) // 1 < 3 -> stays pending
