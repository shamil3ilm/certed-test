...
describe('countPeople', () => {
  ...
  it('counts active students/tutors for dashboard parity with the drill-down modals', async () => {
    const studentClient = { from: vi.fn(() => queryBuilder({ data: [], error: null, count: 42 })) }
    const tutorClient = { from: vi.fn(() => queryBuilder({ data: [], error: null, count: 6 })) }
    const pendingClient = { from: vi.fn(() => queryBuilder({ data: [], error: null, count: 3 })) }
    vi.mocked(createAdminClient)
      .mockReturnValueOnce(studentClient as any)
      .mockReturnValueOnce(tutorClient as any)
      .mockReturnValueOnce(pendingClient as any)

    await countPeople()

    expect(studentClient.from.mock.results[0].value.eq).toHaveBeenCalledWith('role', 'student')
    expect(studentClient.from.mock.results[0].value.eq).toHaveBeenCalledWith('status', 'active')
    expect(tutorClient.from.mock.results[0].value.in).toHaveBeenCalledWith('role', ['tutor', 'mentor'])
    expect(tutorClient.from.mock.results[0].value.eq).toHaveBeenCalledWith('status', 'active')
    expect(pendingClient.from.mock.results[0].value.eq).toHaveBeenCalledWith('status', 'pending')
  })
})
...
describe('completePasswordRegistration', () => {
  ...
  it('returns an internal error when orphan auth-user cleanup fails', async () => {
    const createUser = vi.fn(async () => ({ data: { user: { id: 'auth-1' } }, error: null }))
    const deleteUser = vi.fn(async () => ({ data: {}, error: { message: 'delete failed' } }))
    const authClient = { auth: { admin: { createUser, deleteUser } } }
    vi.mocked(createAdminClient)
      .mockReturnValueOnce(
        makeClient({
  ...
  it('rejects a minor student (guardian on record) without guardian consent', async () => {
  ...
  it('lets a minor student register WITH guardian consent', async () => {
    vi.mocked(setupCodeValid).mockReturnValue(true as any)
