...
vi.mock('@/lib/services/timetable-slots', async (importOriginal) => {
  ...
  return { ...actual, listSlots: (opts?: unknown) => listSlots(opts) }
})
...
beforeEach(() => {
  ...
  requireCapabilityApi.mockImplementation(async (_capability: string) => {
...
describe('POST /api/timetable', () => {
  ...
  it('rejects an invalid slot with 422 (end before start)', async () => {
