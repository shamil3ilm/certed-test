...
2. **Enable the `pg_cron` extension BEFORE applying the chain** — `create extension if not exists pg_cron;` works from a normal SQL session, no dashboard step needed. Four migrations (`0051`, `0058`, `0059`, `0101`) schedule retention jobs inside a guard that SKIPS silently when the extension is absent and still reports success, so a chain applied first leaves a database with **no retention at all** — `audit_log`, read notifications, sent emails and rate-limit rows growing without bound — while "migrations applied" looks true. Verified on a fresh project: without it, zero jobs; enable it and re-run those four, and all four schedules appear. **If you provision from the one-shot rebuild snapshot instead of the numbered chain, enabling the extension is necessary but not sufficient** — the snapshot is a schema-only `pg_dump`, cron schedules are rows in `cron.job` rather than DDL, and nothing re-emits them, so a clean apply with `pg_cron` already installed produces zero jobs (verified). Run [../supabase/provision/04_retention_jobs.sql](../supabase/provision/04_retention_jobs.sql) afterwards.
