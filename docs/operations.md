...
- **Restore drill (do it once, then annually):** a backup you have never restored is a hypothesis. It is now one command — [`scripts/restore-drill.sh`](../scripts/restore-drill.sh) verifies a restored database is at the chain head (guardians / mentee_notes / `rls_disabled_tables` present) and that its receipts reconcile, and prints the elapsed time (your RTO). Restore the latest Supabase backup into a **scratch** project, then run `PGHOST=… PGUSER=… PGDATABASE=<scratch> bash scripts/restore-drill.sh` against it; **separately** confirm the custodial attachments come back from Google Drive (they live outside the DB backup). The mechanics are proven — `bash scripts/restore-drill.sh --rehearse` runs the full build → dump → restore → verify cycle locally (5/5; last re-verified **2026-09-13**, local RTO **~1s**) — but that rehearsal restores a locally-built DB, **not** a real Supabase backup, so the **production drill has not yet been performed and no production RTO is recorded here.** Do it once now, record it below, then annually.
  - **Production drill — not yet performed.** Blocking pre-launch (see the restore-drill gate in [production-checklist.md](production-checklist.md)). The rehearsal mechanism is green (2026-09-02); the one remaining step is an admin with Supabase **Pro** access restoring the latest real backup into a scratch project, running the drill against it, and separately confirming a Drive attachment comes back. Record `date · RTO · attachments-verified` here when done — this line stays "not yet performed" until then.
...
- **The policy pages are UNLINKED while counsel is outstanding.** `/terms` and `/privacy` still exist and still render, and both open with a "Draft: pending legal review — not yet in force" banner. Nothing links to them at all: the links were removed from the marketing footer, the cookie notice, the contact form, the login page, the terms/privacy cross-references and the consent wording. They stay reachable by typing the URL, which is how an owner reads one when needed. An incomplete draft reachable from ordinary navigation reads as a finished document nobody proof-read.
  - **Why they are not withdrawn outright:** registration binds to them. The consent checkbox attests that a guardian "has read and agrees to the Terms of Use and Privacy Policy", and `recordConsent` stamps `TERMS_VERSION` / `PRIVACY_VERSION` into the append-only trail the privacy policy itself promises. An attestation whose documents cannot be reached at all is weaker evidence on a DPDP or GCC subject request than a draft, not stronger — so the consent wording still NAMES both documents, as plain text linking nowhere, and a record of acceptance still identifies what was accepted.
  - **The acceptance flow itself stays, and removing it was tried.** `completePasswordRegistration` gates on the guardian checkbox (`src/lib/services/users/registration.ts`), refusing any account `requiresGuardianConsent` flags — in a KG-12 academy that is every student with no date of birth on record, because the rule fails closed. With no checkbox the form never sends the attestation and those registrations fail outright. Deleting the gate instead is worse: `recordConsentAcceptance(..., { guardianConsent: needsGuardian })` would then log guardian consent as given for a minor nobody attested for, forging the fact the log exists to evidence. Two tests in `tests/unit/services/users.test.ts` hold this line.
  - **They are also `noindex`.** Both pages set `robots: { index: false, follow: false }`, because unlinking retracts nothing a search engine already holds — and both were linked from the marketing footer, so they may already be in an index. Deliberately **not** a `robots.txt` disallow: a crawler barred from fetching the page never sees the directive, so the URL can stay indexed from an external link. `sitemap.ts` never listed them.
