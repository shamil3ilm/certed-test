...
| Where are roles/personas defined?            | Fixed identity: `profiles.role`. Assignable: `persona_assignments`. Read in [src/lib/permission/personas.ts](../src/lib/permission/personas.ts). Changed through guarded database functions: invite and revoke via [src/lib/data/profiles-directory.ts](../src/lib/data/profiles-directory.ts), restore and erase via [src/lib/data/profile-lifecycle.ts](../src/lib/data/profile-lifecycle.ts), mentor access via [src/lib/data/mentorships.ts](../src/lib/data/mentorships.ts); table access in [src/lib/data/personas.ts](../src/lib/data/personas.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Where are resources created?                 | [src/app/(prt)/resources/UploadForm.tsx](<../src/app/(prt)/resources/UploadForm.tsx>) → [src/lib/services/resources/](../src/lib/services/resources/) → [src/lib/data/resources.ts](../src/lib/data/resources.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| How are long staff lists kept navigable?     | By GROUPING under the student, not the class — in a 1:1 academy a class IS (student, subject), so a class-keyed list grows as their product. Two shapes, and the difference is deliberate: `/classroom` and `/session-timings` page the STUDENT ROSTER ([classroom.ts](../src/lib/services/page-data/classroom.ts), [session-timings.ts](../src/lib/services/page-data/session-timings.ts)), so a student's rows are never split across a page and each group caps what it shows with a "See all" drill-in; `/documents` groups WITHIN the page ([document-search.ts](../src/lib/services/page-data/document-search.ts)), because a document belongs to a class and is read through RLS — there is no student-keyed query to page, and its order is recency. Filters are pushed INTO the per-group reads, never applied to an already-sliced list. Surfaces that are deliberately NOT grouped: `/students` and `/admin/users` (the row already IS the entity) and `/admin/history` + `/notifications` (chronological logs, where time is the right axis) |
| Where is the database schema?                | `supabase/migrations/` — the highest-numbered file is the head (do not hard-code it here; it moves every migration). `supabase/rebuild/0000_full_rebuild.sql` is a generated snapshot for one-shot provisioning. Prose: [docs/schema-reference.md](./schema-reference.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
