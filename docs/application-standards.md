...
19. Reusable content sets, policy lists, option sets, and repeated UI copy must move to a shared module once they are used by more than one surface or make a route file materially harder to scan.
...
3. Capability-gated surfaces must use capability guards consistently across page, API, and service layers so overrides do not open one layer while another still denies it.
4. Role-based guards are allowed only for structural rules that are intentionally not override-grantable, for example hard admin-tier operations.
